...
func _build_multiplayer() -> void:
	...
	var top_size := Vector2(screen_size.x / 2, screen_size.y / top_player_count)
	...
	var bottom_size := Vector2(screen_size.x / 2, screen_size.y / bottom_player_count)
	
	# Set the orientation of the containers to horizontal, so the viewports are side by side.
	top.set_vertical(true)
	bottom.set_vertical(true)
